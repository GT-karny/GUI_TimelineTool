...
- **補間モード**
  - ツールバーの `Interpolation` コンボボックスで `cubic` / `linear` / `step` / `bezier` を切り替えます。
  - `bezier` を選択すると各キーフレームに 2 本のハンドルが表示され、ドラッグで制御点の位置を変えられます。横方向の移動でハンドル時刻 (接線の向き)、縦方向で値を調整でき、滑らかな立ち上がりや減衰を表現できます。Inspector からハンドルの `t` / `v` 値を直接入力することも可能です。
- **Inspector での編集**
